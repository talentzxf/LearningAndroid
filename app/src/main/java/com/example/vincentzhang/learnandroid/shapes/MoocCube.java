...
//                    "}";
    ...
    private final String fragmentShaderCode =
            "precision mediump float;" +
                    "const bool useMoocTexture = false;" +
                    noiseFuncions +
                    "varying vec4 frag_pos;" +
                    "varying vec4 normal;" +
                    "varying vec4 v_color;" +
                    ...
                    "if(useMoocTexture){" +
                    "result = (ambient + diffuse).xyz * texture2D(u_Texture, texCoord).xyz; " +
                    "}else{"+
                    "float freq = 10.0;" +
...
//                    "t=sin(t);" +
                     "result = (ambient + diffuse).xyz * marble_color(sin(freq*t))*vec3(1, 0.776, 0.294);" +
...
//                    "}"+
    ...
    public MoocCube() {
        ...
        Bitmap earthTexture = BitmapFactory.decodeResource(OpenGLActivity.getContext().getResources(),
                R.drawable.imooc);
        Shared.textureManager().addTextureId(earthTexture, "imooc_logo");
        mTextureDataHandle = Shared.textureManager().getGlTextureId("imooc_logo");
