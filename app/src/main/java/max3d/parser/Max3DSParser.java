...
import max3d.parser.AParser;
import max3d.parser.IParser;
import max3d.parser.ParseObjectData;
import max3d.parser.ParseObjectFace;
import max3d.vos.Number3d;
import max3d.vos.Uv;
...
public class Max3DSParser extends AParser implements IParser {
	...
	public void parse() {
		...
		Log.d(Max3d.TAG, "Start parsing object");
		...
		try {
			...
			{
				Log.d(Max3d.TAG, "Not a valid .3DS file!");
				return;
			}
			else
			{
				Log.d(Max3d.TAG, "Found a valid .3DS file");
			}
			while(!endReached)
			{
				readChunk(stream);
		...
		Log.d(Max3d.TAG, "End parsing object");
	}
	...
	private void readChunk(InputStream stream) throws IOException {
		...
		case OBJECT_BLOCK:
			...
			Log.d(Max3d.TAG, "Parsing object " + currentObjName);
			break;
		case TRIMESH:
			if(firstObject)
			{
	...
	public Object3dContainer getParsedObject() {
		Log.d(Max3d.TAG, "Start object creation");
		...
		for (int i = 0; i < numObjects; i++) {
			...
			Log.d(Max3d.TAG, "Creating object " + o.name);
		...
		Log.d(Max3d.TAG, "Object creation finished");
